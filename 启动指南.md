...
# 1. 创建新分支（格式：git branch 分支名，分支名建议语义化，如 feature/xxx、bugfix/xxx）
git branch feature/efficientnet-infer  # 示例：创建“高效网推理功能”分支

# 2. 查看所有分支（确认新分支已创建，* 表示当前所在分支）
git branch  # 本地分支列表
# 或 git branch -a  # 查看本地+远程所有分支

# 3. 切换到新创建的分支
git checkout feature/efficientnet-infer