...
2. **识别需要查询/调整的信息类别**：
   ...
   - 如果是调整时间安排 → 生成 planning 类别的任务
3. **设计actionable_tasks**：
   - 每个任务必须是独立的、可执行的
   - 使用"查询"、"搜索"等动词
   - 只关注用户反馈中提到的调整部分
4. **不要重复已有的信息**：
   - 如果用户没有要求调整某个部分，就不要生成相关任务
   - 例如：用户只要求换酒店，就不要重新查询交通
   ...
   **反馈调整任务示例**：
   ...
   "actionable_tasks": [
     {{
       ...
       "summary_task": "对比新查询的酒店与原酒店的价格和性价比，推荐最佳选择"
     }}
   ]
   ```
